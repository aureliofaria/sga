...
export default function PaymentRecurrences() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return paymentsApi.createRecurrence({
        ...
        intervalCount: Math.max(1, parseInt(form.intervalCount, 10) || 1),
        nextRunAt: form.nextRunAt ? new Date(form.nextRunAt).toISOString() : undefined,
      });
    },
    onSuccess: () => { toast.success('Recorrência criada'); setShowForm(false); setForm({ ...emptyForm }); invalidate(); },
    onError: (e: any) => toast.error(e?.response?.data?.error || e?.message || 'Erro ao criar recorrência'),
  });
  ...
  const editAmountMutation = useMutation({
    mutationFn: ({ id, amountCents }: { id: string; amountCents: number }) => paymentsApi.updateRecurrence(id, { amountCents }),
    onSuccess: () => { toast.success('Valor atualizado'); invalidate(); },
    onError: (e: any) => toast.error(e?.response?.data?.error || 'Erro ao atualizar valor'),
  });
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Nova Recorrência</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.paymentCategory} onChange={(e) => setForm({ ...form, paymentCategory: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Primeira execução</label>
              ...
              <p className="text-xs text-gray-400 mt-1">Em branco = agora.</p>
            </div>
            <div className="sm:col-span-2">
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              disabled={!formValid || createMutation.isPending || paymentFlows.length === 0}
                ...
                return (
                  <tr key={rec.id} className={rec.isActive ? '' : 'opacity-60'}>
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button
                        onClick={() => {
                          const v = window.prompt('Novo valor (R$):', (rec.amountCents / 100).toFixed(2));
                          if (v == null) return;
                          const reais = parseFloat(v.replace(/[^0-9.]/g, ''));
                          if (!Number.isFinite(reais) || reais <= 0) { toast.error('Valor inválido'); return; }
                          editAmountMutation.mutate({ id: rec.id, amountCents: Math.round(reais * 100) });
                        }}
                        className="text-golplus-blue-600 hover:underline mr-3"
                      >
                        Editar valor
                      </button>
